...
function ParkingList() {
  ...
  useEffect(() => {
    if (location.state) {
      ...
      fetch(
        `${BACKEND_URL}/nearby-parking?lat=${lat}&lon=${lng}&radius_km=10&vehicle_type=${vehicleType}`,
      )
        .then((response) => {
          if (!response.ok) {
  ...
  if (sortBy === "distance") {
    ...
    const available = sortedSpots
      .filter((s) => s.free_places > 0)
      .sort((a, b) => (b.free_places || 0) - (a.free_places || 0));
    const unavailable = sortedSpots.filter(
      (s) => !s.free_places || s.free_places === 0,
    );
  ...
  if (loading)
    return (
      <div style={{ textAlign: "center", paddingTop: "10rem" }}>Loading...</div>
    );
  if (error)
    return (
      <div style={{ color: "red", textAlign: "center", paddingTop: "10rem" }}>
        Error: {error}
      </div>
    );
  ...
  console.log(
    "Invalid spots:",
    parkingSpots.filter((spot) => !spot.latitude || !spot.longitude),
  );
      ...
      <div style={{ width: "40%", overflowY: "auto", padding: "20px" }}>
        ...
        >
          <span style={{ fontSize: 28, color: "#1976d2" }}>
            <svg width="28" height="28" fill="none" viewBox="0 0 24 24">
              <path
                d="M12 2C7.03 2 3 6.03 3 11c0 5.25 7.5 11 8.1 11.5.18.15.42.15.6 0C13.5 22 21 16.25 21 11c0-4.97-4.03-9-9-9zm0 15c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5z"
                fill="#1976d2"
              />
        ...
        <div>
          {sortedSpots.length > 0 ? (
            sortedSpots.map((spot) => (
              <div
                ...
                style={{
                  ...
                  position: "relative",
                  border:
                    selectedSpot?.id === spot.id ? "2px solid #0d6efd" : "none",
                }}
              >
                    ...
                    <img
                      src={
                        spot.image_url && spot.image_url.trim() !== ""
                          ? spot.image_url
                          : fallbackImages[spot.id % fallbackImages.length]
                      }
                  ...
                  <div className="col-md-8">
                    <div className="card-body d-flex flex-column h-100">
                      <h5 className="card-title">
                        {spot.name || "Parking Garage"}
                      </h5>

                      <p
                        className={`card-text ${spot.free_places === 0 ? "text-danger" : "text-success"}`}
                      >
                        Free Spaces:{" "}
                        {spot.free_places !== undefined
                          ? spot.free_places
                          : "N/A"}
                      </p>
                      ...
                      <p className="card-text mb-auto">
                        <FaWalking />
                        {spot.distance_km
                          ? ` ${(spot.distance_km * 12).toFixed(0)} min`
                          : ""}
                        (
                        {spot.distance_km
                          ? `${spot.distance_km.toFixed(2)} km`
            ...
            <p style={{ color: "#888", textAlign: "center", margin: 32 }}>
              No parking found at the moment.
            </p>
          )}
        </div>
      </div>
